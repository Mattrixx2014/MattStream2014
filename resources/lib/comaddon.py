...
class progress:
    ...
    def VScreate(self, title='', desc='', large=False):
        ...
        # Ne pas afficher le sablier si nous ne sommes pas dans un menu MattStream2014
...
class listitem(xbmcgui.ListItem):
    ...
    def addMenu(self, sFile, sFunction, sTitle, oOutputParameterHandler=False):
        sPluginPath = 'plugin://plugin.video.MattStream2014/'  # cPluginHandler().getPluginPath()
...
def VSlog(e, level=xbmc.LOGDEBUG):
    try:
        ...
        xbmc.log('\t[PLUGIN] MattStream2014: ' + str(e), level)
...
class siteManager:
    ...
    def __init__(self):
        ...
        self.defaultPath = VSPath('special://home/addons/plugin.video.MattStream2014/resources/sites.json')
        self.defaultData = None

        # Propriétés selon le profil        
        name = VSProfil()
        if name == 'Master user':   # Le cas par defaut
            path = VSPath('special://home/userdata/addon_data/plugin.video.MattStream2014/sites.json')
        else:
            path = VSPath('special://home/userdata/profiles/' + name + '/addon_data/plugin.video.MattStream2014/sites.json')
    ...
    def getProperty(self, sourceName, propName):
        ...
        if sourceData:
            ...
            # Propriété inconnue, on récupère la valeur par défaut ...
            defaultProps = self._getDefaultProp(sourceName)
            if propName not in defaultProps:
                return False
