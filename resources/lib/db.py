...
class cDb(object):
    def __enter__(self):
        ...
        try:
            if self.DB_MySql:
                ...
                self.dbcur.execute("SHOW DATABASES LIKE '%s'" % db_name)
                if self.dbcur.fetchone() is None:
                    self.dbcur.execute("CREATE DATABASE %s" % db_name)
                self.dbcur.close()
                self.db.close()
    ...
    def reset_download(self, meta):
        ...
        url = QuotePlus(meta['url'])
        sql_select = "UPDATE download SET status = '0' WHERE status = '1' and url = '%s'" % url
    ...
    def update_download(self, meta):
        ...
        sql_update = "UPDATE download SET size = '%s', totalsize = '%s', status = '%s' WHERE path = '%s'" % (size, totalsize, status, path)
    ...
    def del_download(self, meta):
        ...
        if url:
            sql_delete = "DELETE FROM download WHERE url = '%s'" % url
        else:
            sql_delete = "DELETE FROM download WHERE path = '%s'" % path
